...
function backend(options = {}) {
  ...
  api.SPREADSHEET_ID='test'; api.SHEET_NAME='test';
...
test('sender does not resubmit network errors or declare opaque errors successful',async()=>{
  for(const fetch of [async()=>{throw Error('network');},async()=>({ok:false}),async()=>({ok:true,json:async()=>({result:'error'})})]) {
...
test('timeout aborts the request without a second POST and clears the timer',async()=>{
  ...
  }),true,{setTimeout(callback,delay){assert.equal(delay,20000);expire=callback;return 123;},clearTimeout(id){assert.equal(id,123);cleared=true;}});
