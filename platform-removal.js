(function () {
  ...
  if (!success) {
    ...
    success.innerHTML = '<div class="platform-success-icon" aria-hidden="true">&#10003;</div>' +
      (isFrench
        ...
        : '<h2>Removal request submitted</h2><p>Your ' + platform + ' review details were sent. We will assess the policy concern and contact you about the next step.</p><button class="button" id="platformSuccessClose" type="button">Send another request</button>');
  ...
  function openModal(trigger) {
    ...
    window.setTimeout(function () { formQuantity.focus(); }, 150);
  }
  function closeModal() {
    if (!usePopupForm) return;
  ...
  if (usePopupForm) {
    ...
    formQuantityField.classList.add('full', 'modal-quantity-field');
    form.querySelector('.field-grid').prepend(formQuantityField);
